...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Check onboarding — localStorage is source of truth for returning users
    const done = (() => {
      try { return !!localStorage.getItem('lumina_onboarding_done') } catch { return false }
    })()
    if (!done && !onboardingComplete) {
      // New user — check if they actually have a Supabase session first
      const supabase = createClient()
      supabase.auth.getUser().then(({ data: { user } }) => {
        if (user) {
          router.replace('/onboarding')
        } else {
          // Not logged in yet — show home normally (auth middleware handles protection)
          setIsReady(true)
        }
      })
      return
          ...
          {isReady && (
            ...
            >
              ...
              <motion.div
                ...
                className="px-5 mt-5"
              >
                <p className="text-[10px] uppercase tracking-[0.12em] text-lumina-purple-dream/60 mb-1.5">
                  Your sanctuary
                </p>
                <h1 className="text-[26px] font-semibold leading-tight text-dreamy">
                  A space made<br />just for you.
                </h1>
                <p className="text-sm font-light text-white/35 mt-2 leading-relaxed">
